...
public class GroppingStreamExam {
	...
	public static void main(String[] args) {
		...
		Map<String, List<Student>> map = students.stream().collect(Collectors.groupingBy(Student::getMajor));
		System.out.println(map);

		System.out.println("2. 전공별 학생수 계산(counting)- {Computer Science=2, Mathematics=2, Physics=1}");
		Map<String, Long> map2 = students.stream().collect(Collectors.groupingBy(Student::getMajor, Collectors.counting()));

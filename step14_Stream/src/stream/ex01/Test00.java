...
//		}
		...
		//2.Stream이용방식으로 해보자
		List<Student> finalList1 = scoreList.stream()
		.filter(new Predicate<Student>() {
			@Override
			public boolean test(Student t) {
				if(t.getScore() >=80) return true;
				return false;
			}
		...
		List<String> finalList2 = students.stream()
				.filter((s) -> s.getScore() >=80)
				.sorted(Comparator.comparingDouble(Student::getScore))
				.map(Student::getName)
				.toList();
		
		System.out.println("80점 넘는 학생들 수 : " + finalList2.size());
